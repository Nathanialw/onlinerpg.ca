  ...
  void Update_Enemies(Game::State &game) {
    ...
    for (int i = 1; i < units.size(); i++) {
      ...
      if (Check_For_Target(units[i].position, player.position)) {
        ...
        // calculate next cell
        Component::Position moveTo = Pathing::Move_To(game.map[player.level][player.location].pathing, units[i].position, player.position);
        ...
        if (Map::Get_Adjacent_Tile(game, former.x + moveTo.x, former.y + moveTo.y).at(0) == Spawn::Get_Unit_Char(player.def.species)) {
          ...
          Attack::Melee(game.objects, game.map[player.level][player.location].defaultChunk, game.map[player.level][player.location].chunk, former.x, former.y, moveTo.x, moveTo.y);
          continue;
        }
        ...
        auto map = Map::Get_Map(game.map[player.level][player.location].chunk);
        Pathing::Update(game.map[player.level][player.location].pathing, map);
