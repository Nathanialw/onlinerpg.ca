...
namespace Send {
  ...
  void Init(const websocketpp::connection_hdl &hdl, const std::basic_string<char> &msg, websocketpp::server<websocketpp::config::asio> &print_server, Game::State &game) {
    ...
    std::string map = Map::Init(game.map[player.level][player.location].defaultChunk, game.map[player.level][player.location].chunk, game.map[player.level][player.location].rooms, game.seed);
    Pathing::Init(game.map[player.level][player.location].pathing, map);
    Player::Spawn(game, msg);
    Spawn::Init(game.map[player.level][player.location].chunk, game.map[player.level][player.location].rooms, game.objects); // msg->get_payload()
